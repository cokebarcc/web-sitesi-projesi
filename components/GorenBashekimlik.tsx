  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-24">
      ...
      <div className="bg-white p-10 rounded-[48px] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">GÖREN Başarı Sıralaması</h2>
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.2em] mt-1">İller Arası Stratejik Performans Kıyaslama</p>
        </div>
        <label className="bg-slate-900 text-white px-10 py-5 rounded-3xl font-black text-xs shadow-xl cursor-pointer hover:bg-slate-800 transition-all active:scale-95 flex items-center gap-3">
      ...
      {cityResults.length > 0 ? (
        <div className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end">
            <PodiumCard rank={2} data={cityResults[1]} color="bg-slate-100 text-slate-600" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-7 bg-white rounded-[48px] shadow-xl border border-slate-100 overflow-hidden">
              <div className="p-8 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
                <h3 className="text-xl font-black text-slate-900 uppercase">Tüm İller Sıralaması</h3>
                <span className="bg-slate-900 text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest">{cityResults.length} İL ANALİZ EDİLDİ</span>
              </div>
              <div className="overflow-y-auto max-h-[700px] custom-scrollbar">
                <table className="w-full text-left">
                  <thead className="bg-slate-50 sticky top-0 z-10">
                    <tr>
                      <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">SIRA</th>
                      <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">İL ADI</th>
                      <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">TOPLAM PUAN</th>
                      <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">BAŞARI %</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {cityResults.map((res) => (
                      <tr 
                        ...
                        className={`cursor-pointer transition-all ${selectedCityName === res.city ? 'bg-blue-50/50' : 'hover:bg-slate-50'}`}
                      >
                        <td className="px-8 py-5 text-center">
                          <span className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs ${res.rank! <= 3 ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-400'}`}>
                            {res.rank}
                          </span>
                        </td>
                        <td className="px-8 py-5">
                          <p className="font-black text-slate-900 uppercase text-sm">{res.city}</p>
                        </td>
                        <td className="px-8 py-5 text-center font-bold text-slate-600">{res.totalScore}</td>
                        <td className="px-8 py-5 text-center">
            ...
            <div className="lg:col-span-5 space-y-8">
              {selectedCity ? (
                <div className="bg-slate-900 text-white p-12 rounded-[56px] shadow-2xl relative overflow-hidden sticky top-10">
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-10">
                      ...
                      <div className="bg-white/10 px-6 py-4 rounded-3xl border border-white/10 text-center">
                        <p className="text-[10px] font-black text-slate-400 uppercase mb-1">GENEL SIRA</p>
                        <p className="text-3xl font-black text-white">#{selectedCity.rank}</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-6 mb-12">
                       <div className="p-6 bg-white/5 rounded-3xl border border-white/5">
                          <p className="text-[9px] font-black text-slate-400 uppercase mb-2">TOPLAM PUAN</p>
                          <p className="text-3xl font-black text-emerald-400">{selectedCity.totalScore}</p>
                       </div>
                       <div className="p-6 bg-white/5 rounded-3xl border border-white/5">
                          <p className="text-[9px] font-black text-slate-400 uppercase mb-2">BAŞARI ORANI</p>
                          <p className="text-3xl font-black text-amber-400">%{selectedCity.achievementRate}</p>
                       </div>
                    </div>

                    <div className="space-y-6">
                       <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest border-b border-white/10 pb-4">GÖSTERGE BAZLI KIYASLAMA</h4>
                       ...
                       <div className="flex justify-center gap-6 mt-4">
                          <div className="flex items-center gap-2">
                             ...
                             <span className="text-[10px] font-black text-slate-400 uppercase">İL PUANI</span>
                          </div>
                          <div className="flex items-center gap-2">
                             <div className="w-3 h-3 bg-amber-500 rounded opacity-50"></div>
                             <span className="text-[10px] font-black text-slate-400 uppercase">TR ORTALAMASI</span>
                          </div>
                       </div>
                    </div>
                  </div>
                ...
                <div className="bg-white p-20 rounded-[48px] border-2 border-dashed border-slate-200 text-center">
                  <p className="text-slate-400 font-bold italic">Detaylı analiz için sıralamadan bir il seçiniz.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white p-12 rounded-[56px] shadow-xl border border-slate-100">
             <div className="flex justify-between items-center mb-10">
                <h3 className="text-xl font-black text-slate-900 uppercase">İL BAZLI GENEL BAŞARI DAĞILIMI</h3>
                <div className="flex gap-4">
                   <div className="flex items-center gap-2">
                      <div className="w-3 h-3 bg-emerald-500 rounded"></div>
                      <span className="text-[10px] font-black text-slate-500 uppercase">YÜKSEK BAŞARI</span>
                   </div>
                   <div className="flex items-center gap-2">
                      <div className="w-3 h-3 bg-rose-500 rounded"></div>
                      <span className="text-[10px] font-black text-slate-500 uppercase">GELİŞTİRİLMELİ</span>
                   </div>
                </div>
             </div>
        ...
        <div className="bg-white p-32 rounded-[56px] border-2 border-dashed border-slate-200 text-center flex flex-col items-center gap-8">
           ...
           <div>
             <h4 className="text-2xl font-black text-slate-400 uppercase tracking-widest">Matris Verisi Bekleniyor</h4>
             <p className="text-slate-400 font-medium max-w-md mx-auto mt-2 italic">Görselde paylaştığınız; İllerin satırlarda, göstergelerin sütunlarda olduğu Excel tablosunu yükleyiniz.</p>
           </div>
        </div>
      )}
    </div>
...
const PodiumCard = ({ rank, data, color, isWinner }: { rank: number; data: CityScore; color: string; isWinner?: boolean }) => {
  if (!data) return null;
  return (
    <div className={`relative flex flex-col items-center p-8 rounded-[48px] shadow-lg border transition-all hover:scale-105 ${isWinner ? 'scale-110 ring-4 ring-amber-400/30' : ''} ${color}`}>
       <div className={`w-12 h-12 rounded-full flex items-center justify-center font-black text-xl mb-4 shadow-inner ${isWinner ? 'bg-amber-400 text-white' : 'bg-white text-slate-400'}`}>
          {rank}
       </div>
