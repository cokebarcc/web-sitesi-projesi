...
const ScheduleTable: React.FC<ScheduleTableProps> = ({ data, selectedBranch, onImportExcel, onDelete, onClearMonth }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 flex flex-col lg:flex-row justify-between items-center gap-6">
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
          <div className="flex flex-col gap-1 w-full sm:w-auto">
            ...
            <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3 text-sm font-black outline-none cursor-pointer hover:bg-slate-100 transition-colors w-40">
              {MONTHS.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-1 w-full sm:w-auto">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">DÖNEM YILI</label>
            <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3 text-sm font-black outline-none cursor-pointer hover:bg-slate-100 transition-colors w-32">
      ...
      <div className="bg-white rounded-[40px] shadow-xl border border-slate-100 overflow-hidden">
        <div className="p-10 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white gap-4">
          <div>
            <h4 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Çalışma Cetveli: {selectedMonth} {selectedYear}</h4>
            <p className="text-slate-500 text-xs mt-1 font-bold tracking-wide">Hekimlerin {selectedMonth} {selectedYear} dönemine ait tüm faaliyetleri aşağıda listelenmiştir.</p>
          </div>
          <div className="text-right bg-slate-50 p-6 rounded-3xl shadow-sm border border-slate-100 min-w-[140px] flex flex-col items-center">
            <span className="text-4xl font-black text-blue-600 leading-none">{sortedData.length}</span>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Toplam Kayıt</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                ...
                <th className="px-10 py-6 text-[11px] font-black text-slate-400 uppercase tracking-widest">Aksiyon / Faaliyet</th>
                <th className="px-10 py-6 text-[11px] font-black text-slate-400 uppercase tracking-widest text-center">Gün Sayısı</th>
                <th className="px-10 py-6 text-[11px] font-black text-slate-400 uppercase tracking-widest">MHRS Kapasite</th>
              ...
              {sortedData.length > 0 ? sortedData.map((item) => (
                <tr key={item.id} className="hover:bg-blue-50/20 transition-colors group">
                  <td className="px-10 py-6">
                    <p className="font-black text-slate-900 uppercase leading-tight text-sm">{item.doctorName}</p>
                  ...
                  <td className="px-10 py-6 text-center font-black text-slate-700 text-sm">{item.daysCount} GÜN</td>
                  <td className="px-10 py-6">
                  ...
                  <td className="px-10 py-6 text-center">
                    <button onClick={() => { if(confirmingId === item.id) { onDelete(item.id); setConfirmingId(null); } else setConfirmingId(item.id); }} 
                      className={`px-6 py-2.5 rounded-2xl transition-all font-black text-[10px] border shadow-sm ${confirmingId === item.id ? 'bg-rose-600 text-white border-rose-500' : 'bg-white text-slate-300 hover:text-rose-600 border-slate-100'}`}>
                ...
                <tr>
                  <td colSpan={5} className="px-10 py-48 text-center text-slate-300">
                    <div className="flex flex-col items-center gap-8">
                      <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center shadow-inner">
                        <svg className="w-12 h-12 text-slate-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                      </div>
                      <p className="font-black italic text-2xl uppercase tracking-[0.2em] text-slate-400 opacity-60">{selectedMonth} {selectedYear} İÇİN VERİ BULUNAMADI</p>
                    </div>
                  </td>
                </tr>
              )}
