    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-8 animate-in fade-in duration-700">
        <div className="bg-white p-20 rounded-[48px] border-2 border-dashed text-center max-w-2xl shadow-sm" style={{ borderColor: 'var(--border-2)' }}>
          ...
          <h3 className="text-2xl font-black tracking-tight mb-3" style={{ color: 'var(--text-1)' }}>Hizmet Girişim SUT Analizörü</h3>
          <p className="mb-8 font-medium leading-relaxed" style={{ color: 'var(--text-3)' }}>Hastanelerin SUT kodlu girişimsel faaliyetlerini karşılaştırmak için Excel dosyasını yükleyiniz.</p>
            ...
            {aiAnalysis.split('\n').map((line, lineIdx) => {
              ...
              return (
                <div key={lineIdx} className={`mb-4 flex gap-4 ${isDanger ? 'bg-rose-500/10 p-5 rounded-2xl border border-rose-500/20' : ''}`}>
                   ...
                   <p className={`text-base leading-relaxed font-medium ${isDanger ? 'text-rose-50' : ''}`} style={!isDanger ? { color: 'var(--text-2)' } : undefined}>{currentLine.replace(/^\*+/, '').trim()}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
           <div>
             <div className="flex items-center gap-3 mb-1">
               ...
               <h3 className="text-2xl font-black tracking-tight" style={{ color: 'var(--text-1)' }}>Risk Grubundaki İşlemler</h3>
             </div>
             <p className="text-sm font-medium" style={{ color: 'var(--text-3)' }}>Ortalamanın 2.5 katından fazla olan anomaliler (Filtrelenebilir ve Sıralanabilir)</p>
           </div>
           ...
           <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
             <div className="relative">
               ...
               />
               <svg className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-3)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
             </div>
            ...
            <tbody className="divide-y" style={{ '--tw-divide-color': 'var(--border-2)' } as React.CSSProperties}>
              {filteredAndSortedRisks.length > 0 ? filteredAndSortedRisks.map((item, idx) => (
                <tr key={idx} className="hover:bg-rose-50/30 transition-colors group">
                  ...
                  <td className="px-8 py-6">
                    <p className="font-bold text-sm leading-tight group-hover:text-rose-600 transition-colors uppercase" style={{ color: 'var(--text-1)' }}>{item.name}</p>
                    <span className="text-[10px] font-black tracking-widest" style={{ color: 'var(--text-3)' }}>{item.code}</span>
                  </td>
                  <td className="px-8 py-6 text-center">
                    <span className="text-lg font-black" style={{ color: 'var(--text-1)' }}>{item.count.toLocaleString('tr-TR')}</span>
                  </td>
                  <td className="px-8 py-6 text-center">
                    <span className="text-sm font-bold" style={{ color: 'var(--text-3)' }}>{item.avg}</span>
                  </td>
                  <td className="px-8 py-6 text-center">
                ...
                <tr>
                  <td colSpan={6} className="px-8 py-20 text-center">
                    <p className="font-black uppercase tracking-[0.2em]" style={{ color: 'var(--text-2)' }}>Kayıt bulunamadı.</p>
                  </td>
                </tr>
              )}
            </tbody>
...
const SortableHeader = ({ label, sortKey, currentSort, onSort, center, highlight }: { 
  ...
  return (
    ...
    >
      <div className={`inline-flex items-center gap-2 ${center ? 'justify-center' : ''} ${highlight ? 'text-rose-600' : ''}`} style={!highlight ? { color: 'var(--text-3)' } : undefined}>
