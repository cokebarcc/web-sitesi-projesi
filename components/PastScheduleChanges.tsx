...
const PastScheduleChanges: React.FC<PastScheduleChangesProps> = ({ versions, setVersions, selectedBranch }) => {
  ...
  return (
    <div className="space-y-10 pb-20 animate-in fade-in duration-700">
      <div className="bg-white p-8 lg:p-12 rounded-[48px] shadow-xl border border-slate-100 relative overflow-hidden">
        ...
        <div className="relative z-10 grid grid-cols-1 xl:grid-cols-12 gap-8 items-center">
          <div className="xl:col-span-4 space-y-6">
            <div>
              <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase leading-tight">CETVEL KIYASLAMA MERKEZİ</h2>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Sürümler Arası Net Kapasite Değişimi</p>
            </div>
            <div className="flex gap-2">
              <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3.5 font-black text-xs outline-none cursor-pointer uppercase transition-colors hover:border-indigo-200">
                {MONTHS.map(m => <option key={m} value={m}>{m}</option>)}
              </select>
              <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="w-28 bg-slate-900 text-white rounded-2xl px-4 py-3.5 font-black text-xs outline-none cursor-pointer transition-transform active:scale-95">
          ...
          <div className="xl:col-span-5 grid grid-cols-1 md:grid-cols-11 gap-4 items-center">
             <div className="md:col-span-5 flex flex-col gap-2">
               <label className="text-[10px] font-black text-slate-400 uppercase ml-1 tracking-widest">İLK CETVEL (BAŞLANGIÇ)</label>
               <select value={baselineLabel} onChange={(e) => setBaselineLabel(e.target.value)} className="bg-white border-2 border-slate-200 rounded-2xl px-4 py-3.5 text-xs font-black outline-none w-full transition-all focus:border-indigo-500 shadow-sm">
      ...
      {isProcessing && (
        <div className="bg-white p-24 rounded-[48px] shadow-sm border border-slate-100 flex flex-col items-center gap-8 animate-in zoom-in-95">
           ...
           <div className="text-center space-y-2">
             <p className="font-black text-xl text-slate-900 uppercase tracking-tight italic">Dosya Analiz Ediliyor...</p>
             <p className="text-slate-400 font-bold text-sm">Hekim bazlı kapasite ve aksiyon verileri işleniyor.</p>
           </div>
        </div>
      )}
      ...
      {comparison && !isProcessing && (
        <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
          <div className="bg-slate-900 text-white p-12 rounded-[56px] shadow-2xl relative overflow-hidden">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative z-10">
                <div className="text-center md:text-left">
                  <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.3em] mb-4">BAŞLANGIÇ KAPASİTESİ</p>
                  <h3 className="text-5xl font-black">{comparison.totalBaseCap.toLocaleString('tr-TR')}</h3>
                  <p className="text-slate-500 text-xs font-bold mt-2 italic">{baselineLabel}</p>
                </div>
                <div className="flex items-center justify-center">
                   <div className={`px-10 py-6 rounded-[32px] border-2 text-center min-w-[220px] transition-all hover:scale-105 ${ (comparison.totalUpdCap - comparison.totalBaseCap) >= 0 ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-rose-500/10 border-rose-500/30 text-rose-400 shadow-rose-900/20 shadow-xl'}`}>
                      <p className="text-[10px] font-black uppercase tracking-widest mb-1 text-slate-400">NET FARK</p>
                ...
                <div className="text-center md:text-right">
                  ...
                  <p className="text-slate-500 text-xs font-bold mt-2 italic">{updatedLabel}</p>
                </div>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                   {comparison.topBranches.map((br, idx) => (
                     <div key={br.name} className="flex items-center gap-6">
                       <span className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center font-black text-xs text-slate-400">{idx+1}</span>
                       <div className="flex-1">
                          <div className="flex justify-between mb-2">
                             <span className="text-[11px] font-black uppercase text-slate-700">{br.name}</span>
             ...
             <div className="bg-white p-10 rounded-[48px] shadow-sm border border-slate-100 relative">
                <h3 className="text-lg font-black text-slate-900 uppercase mb-8 flex items-center gap-3">
                ...
                <div className="space-y-6">
                    {comparison.topPhysDrivers.map((res, idx) => (
                      <div key={res.physician_id} className="flex items-center gap-6 p-5 rounded-[32px] bg-slate-50/50 hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100">
                          <span className="w-10 h-10 rounded-2xl bg-white shadow-sm flex items-center justify-center font-black text-xs text-slate-400">{idx+1}</span>
                          <div className="flex-1">
                            <div className="flex justify-between mb-1">
                                <span className="text-[12px] font-black uppercase text-slate-900">{res.physician_name_display}</span>
                            ...
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{res.branch_display}</p>
                            <div className="mt-3 flex flex-wrap gap-2">
                                <span className="text-[8px] px-2 py-1 rounded bg-slate-900 text-white font-black uppercase shadow-sm">{res.change_type}</span>
          ...
          <div className="bg-white rounded-[48px] shadow-2xl border border-slate-100 overflow-hidden">
            <div className="p-10 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
               <div>
                  ...
                  <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-2">Toplam {comparison.phys_compare.length} hekimde anlamlı değişim bulundu</p>
               </div>
            </div>
            <div className="overflow-x-auto">
               <table className="w-full text-left">
                 <thead className="bg-slate-50">
                   <tr>
                     <th className="px-8 py-5 text-[11px] font-black text-slate-400 uppercase tracking-widest">Hekim & Branş</th>
                     <th className="px-8 py-5 text-[11px] font-black text-slate-400 uppercase tracking-widest text-center">Eski Kap</th>
                     <th className="px-8 py-5 text-[11px] font-black text-slate-400 uppercase tracking-widest text-center">Yeni Kap</th>
                     <th className="px-8 py-5 text-[11px] font-black text-slate-400 uppercase tracking-widest text-center">Fark</th>
                     <th className="px-8 py-5 text-[11px] font-black text-slate-400 uppercase tracking-widest">Aksiyon Farkları (Gün)</th>
                 ...
                 <tbody className="divide-y divide-slate-50">
                   {comparison.phys_compare.map((p) => (
                     <React.Fragment key={p.physician_id}>
                       <tr className={`hover:bg-slate-50/80 transition-colors group cursor-pointer ${expandedDoc === p.physician_id ? 'bg-slate-50' : ''}`} onClick={() => setExpandedDoc(expandedDoc === p.physician_id ? null : p.physician_id)}>
                         <td className="px-8 py-6">
                            <p className="font-black text-slate-900 uppercase text-xs">{p.physician_name_display}</p>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mt-1">{p.branch_display}</p>
                         </td>
                         <td className="px-8 py-6 text-center text-xs font-bold text-slate-300">{p.baseline_capacity}</td>
                         <td className="px-8 py-6 text-center text-xs font-black text-slate-700">{p.updated_capacity}</td>
                         ...
                         <td className="px-8 py-6">
                            <div className="flex flex-wrap gap-2">
                               ...
                               )) : <span className="text-[9px] font-bold text-slate-300 italic">Değişim Yok</span>}
                            </div>
                         </td>
                         <td className="px-8 py-6 text-right">
                           <div className={`p-2 rounded-full bg-slate-100 transition-transform ${expandedDoc === p.physician_id ? 'rotate-180' : ''}`}>
                       ...
                       {expandedDoc === p.physician_id && (
                         <tr className="bg-slate-50/50 animate-in slide-in-from-top-2 duration-300">
                           <td colSpan={6} className="px-12 py-10">
                              <div className="bg-white rounded-[32px] p-8 shadow-inner border border-slate-200">
                                 <div className="flex justify-between items-center mb-8 border-b pb-4">
                                   <h5 className="text-sm font-black text-slate-900 uppercase">Detaylı Denetim İzi</h5>
                                   <span className="text-[9px] font-black px-3 py-1 rounded bg-slate-900 text-white uppercase">{p.change_type}</span>
                                 </div>
                                 <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                                    <div className="space-y-4">
                                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{baselineLabel} ({p.baseline_raw?.rawRows?.length || 0} Satır)</p>
                                       <div className="max-h-[300px] overflow-y-auto custom-scrollbar border rounded-2xl">
                                          <table className="w-full text-[10px]">
                                             <thead className="bg-slate-50 sticky top-0"><tr><th className="p-3 text-left">Tarih</th><th className="p-3 text-left">Saat</th><th className="p-3 text-left">Aksiyon</th><th className="p-3 text-center">Kap.</th></tr></thead>
                                             <tbody className="divide-y divide-slate-100">
                                               {p.baseline_raw?.rawRows?.map((r: any, idx: number) => (
                                                 <tr key={idx} className="hover:bg-slate-50">
                                                    <td className="p-3 font-bold text-slate-600">{r.startDate}</td>
                                                    <td className="p-3 text-slate-400">{r.startTime}</td>
                                                    <td className="p-3 uppercase font-bold text-slate-600">{r.action}</td>
      ...
      {availableVersions.length < 1 && !isProcessing && (
        <div className="bg-white p-32 rounded-[56px] border-2 border-dashed border-slate-200 text-center flex flex-col items-center gap-8 shadow-inner">
           ...
           <div>
             <h4 className="text-2xl font-black text-slate-400 uppercase tracking-widest">KIYASLANACAK VERİ BULUNAMADI</h4>
             <p className="text-slate-400 font-medium max-w-md mx-auto mt-2 italic">Lütfen sağ taraftaki butondan "Sürüm 1" ve "Sürüm 2" olarak iki farklı cetvel yükleyiniz.</p>
           </div>
        </div>
      )}
    </div>
