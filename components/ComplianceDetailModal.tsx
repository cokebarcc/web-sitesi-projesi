...
const ComplianceDetailModal: React.FC<ComplianceDetailModalProps> = ({ isOpen, onClose, row, result }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative border rounded-[20px] shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col mx-4 backdrop-blur-xl" style={{ background: 'var(--surface-1)', borderColor: 'var(--border-1)' }}>
        {/* Header */}
        <div className={`px-6 py-4 border-b flex items-center justify-between ${status.bg}`} style={{ borderColor: 'var(--border-1)' }}>
          <div className="flex items-center gap-3">
            <span className={`px-3 py-1 rounded-xl text-xs font-black ${status.color} ${status.bg} border ${status.border}`}>
              {status.label}
            </span>
            <div>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <div key={item.label} className="rounded-xl px-3 py-2" style={{ background: 'var(--surface-2)' }}>
              ...
              {(() => {
                ...
                return (
                  <div className="rounded-xl px-4 py-3" style={{ background: 'var(--surface-2)' }}>
                    ...
                    {puanFark !== 0 && (
                      <div className="flex justify-between text-sm mt-1 pt-1 border-t" style={{ borderColor: 'var(--border-1)' }}>
                      ...
                      <div className="mt-2 pt-2 border-t border-red-500/10">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {ihlal.fromSectionHeader && (
                            <span className="text-[9px] font-medium border px-1.5 py-0.5 rounded-lg" style={{ color: 'var(--text-muted)', background: 'var(--surface-2)', borderColor: 'var(--border-1)' }}>
                              Bölüm Başlığı
                            </span>
                          )}
                        </div>
                      ...
                      <div className="mt-3 pt-3 border-t border-red-500/10">
                        ...
                        <div className="space-y-1.5">
                          {ihlal.cakisanIslemler.map((ci, ciIdx) => (
                            <div key={ciIdx} className="bg-orange-500/5 border border-orange-500/15 rounded-xl px-3 py-2">
                              <div className="flex items-center gap-2 mb-1">
        ...
        {/* Footer */}
        <div className="px-6 py-3 border-t flex justify-end" style={{ borderColor: 'var(--border-1)' }}>
          <button
            onClick={onClose}
      ...
      <style>{`
        ...
        .custom-scrollbar-modal::-webkit-scrollbar-thumb { background: var(--border-1); border-radius: 3px; }
        .custom-scrollbar-modal::-webkit-scrollbar-thumb:hover { background: var(--text-muted); }
      `}</style>
    </div>
  );
};
